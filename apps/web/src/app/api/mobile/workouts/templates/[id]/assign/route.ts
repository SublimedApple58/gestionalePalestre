...
import { assignTemplate } from "@/lib/services/workout-template-service";
...
/**
 * POST /api/mobile/workouts/templates/[id]/assign
 * Body: { userIds: string[] }
 * 200: { assigned: string[], alreadyAssigned: string[] }
 *
 * Idempotente. Audit log per ogni nuovo userId.
 */
export const POST = withMobileAuth<{ id: string }>(
  async (request, { params, user }) => {
    ...
    try {
      const result = await assignTemplate(db, {
